...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha, gamma=2.0, eps=1e-8, ignore_index=30):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.eps = eps
    ...
    def forward(self, input, target):
        ...
        input_soft = input_soft.view(len(self.alpha), -1)
        input_soft = input_soft[target.view(-1), torch.arange(input_soft.size(1))]

        weight = torch.pow(1.0 - input_soft, self.gamma)
        weight = weight.view(*target.shape)

        # alpha-balance
        balance = self.alpha[target]

        assert balance.shape == loss.shape == weight.shape

        loss = (balance * loss * weight).sum(dim=[1, 2]) / balance.sum(dim=[1, 2])
        loss = loss.mean()
